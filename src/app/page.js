...
export default function Home() {
  ...
  // Estado para loading screen
  const [showLoading, setShowLoading] = useState(true);

  // Forzar scroll al top al cargar/refrescar página
  useEffect(() => {
    // Scroll inmediato al top
    window.scrollTo(0, 0);

    // También prevenir restauración de scroll del navegador
  ...
  // Detectar orientación y setear altura inicial del hero
  useEffect(() => {
    function checkOrientation() {
      // Use consistent breakpoint: mobile if width < 768px
      setIsPortrait(window.innerWidth < 768);
    }
    // Setear altura del hero solo una vez al montar
  ...
  useEffect(() => {
    function handleScroll() {
      ...
      const fadeStart = heroHeight * 0.7; // Comienza a fade cuando está 30% del hero
      const fadeEnd = heroHeight * 0.825;   // Completamente invisible al 80% del hero
  ...
  return (
    <div className={styles.container}>
      ...
      {/* Menú hamburguesa móvil - siempre visible pero detrás del loading overlay */}
      {isPortrait && (
        <>
          <motion.button
            ...
            style={{
              ...
              zIndex: 100, // Por debajo del loading overlay (que es 10000)
              position: 'fixed',
              top: '20px',
              left: '50%',
              ...
              <motion.div
                ...
                exit={{
                  ...
                  transition: {
                    duration: 0.1,
                    ease: "easeInOut",
                    delay: 0 // Sin delay, desaparece inmediatamente
                  }
                }}
              >
      ...
      {/* Navbar desktop - siempre presente */}
      {!isPortrait && <Navbar />}
      ...
      >
        <div className={styles.heroLegend}>
          <span className={styles.heroLegendMobile}>
            En Zeratype <span className={styles.heroLegendStrong}>conectamos</span> marcas, creadores y audiencias para construir comunidades auténticas y conversaciones relevantes.
          </span>
        </div>
        <FloatingShape />
      </section>
      ...
      <div className={styles.sectionsWrapper}>
        <ResizableSection ref={mediaRef} className={styles.section} id="media-building" label={null} color="#212121">
          <MediaBuildingContent showLoading={showLoading} />
        </ResizableSection>
        <ResizableSection ref={hechoRef} className={`${styles.section} ${styles.hechoConSection}`} id="hecho-con" label={null} color="#fff">
          {/* Logo posicionado absolutamente en la línea divisoria */}
          <div className={styles.intersectionLogo}>
      ...
      <div className={styles.footer}>
        <div className={styles.footerRoot}>
          <div className={`${styles.grid} ${styles.gridMobile}`}>
            <div className={`${styles.footerContent} ${styles.col6} ${styles.mobileCol2}`}>
...
function MediaBuildingContent({ showLoading }) {
  ...
  // COMMENTED OUT - Heart particles effect usando CSS puro
  // React.useEffect(() => {
  //   const container = document.getElementById('media-hearts');
  //   if (!container) return;
  ...
  //     // SVG heart shape plano con color #2B2B2B - más grande
  //     heart.innerHTML = `
  //       <svg width="28" height="25" viewBox="0 0 640 640" xmlns="http://www.w3.org/2000/svg">
  //         <path d="M305 151.1L320 171.8L335 151.1C360 116.5 400.2 96 442.9 96C516.4 96 576 155.6 576 229.1L576 231.7C576 343.9 436.1 474.2 363.1 529.9C350.7 539.3 335.5 544 320 544C304.5 544 289.2 539.4 276.9 529.9C203.9 474.2 64 343.9 64 231.7L64 229.1C64 155.6 123.6 96 197.1 96C239.8 96 280 116.5 305 151.1z" fill="#2B2B2B"/>
  //       </svg>
  //     `;

  //     // Posición aleatoria en X dentro del contenedor
  //     heart.style.left = Math.random() * 95 + '%';

  //     // Tamaño aleatorio aún más grande
  //     const scale = Math.random() * 1.5 + 1.5; // 1.5-3x (más grandes)
  //     heart.style.setProperty('--heart-scale', scale);

  //     // Velocidad vertical variable según loading state
  //     const duration = showLoading 
  //       ? Math.random() * 3 + 2  // 2-5 segundos (rápido durante loading)
  //       : Math.random() * 60 + 40; // 40-100 segundos (lento después)
  ...
  //   // Función para crear corazón en posición específica (para pre-población)
  //   const createHeartAtPosition = (yPercent) => {
  //     const heart = document.createElement('div');
  //     heart.className = 'floating-heart';
  ...
  //     // Tamaño aleatorio aún más grande
  //     const scale = Math.random() * 1.5 + 1.5; // 1.5-3x
  //     heart.style.setProperty('--heart-scale', scale);

  //     // Sin animación para los corazones pre-poblados
  //     heart.style.position = 'absolute';
  //     heart.style.opacity = Math.random() * 0.4 + 0.4; // 0.4-0.8

  //     container.appendChild(heart);

  //     // Remover después de un tiempo para que no se acumulen
  //     setTimeout(() => {
  //       if (heart && heart.parentNode) {
  //         heart.parentNode.removeChild(heart);
  //       }
  ...
  //   // Crear corazones animados continuamente (velocidad constante)
  //   window.heartInterval = setInterval(createHeart, 2000); // Velocidad original constante

  //   // No limpiar el intervalo cuando el componente se desmonta
  //   // para que siga funcionando incluso si el usuario navega
  //   return () => {
  //     // Solo limpiamos el contenedor si es necesario
  //     // pero mantenemos el intervalo corriendo
  //   };
  // }, []);
  ...
  return (
    // COMMENTED OUT - Hearts container covering entire section background
    // <>
    //   <div
    //     id="media-hearts"
    //     style={{
    ...
    <div className={styles.mediaBuildingRoot}>
      <div className={`${styles.grid} ${styles.gridMobile}`} style={{ position: 'relative', zIndex: 2 }}>
        ...
        <div className={styles.col12}>
          <div className={styles.dropdownList}>
            <Dropdown title={<span className={styles.dropdownTitleNormal}><b>Creadores</b></span>}>
              <div className={styles.dropdownNewContent}>
                <div className={styles.dropdownImage}>
                  <img src="/image/mediaBuild_creadores.png" alt="Creadores" />
                </div>
            ...
            <Dropdown title={<span className={styles.dropdownTitleNormal}><b>Estudios</b></span>}>
              <div className={styles.dropdownNewContent}>
                <div className={styles.dropdownImage}>
                  <img src="/image/mediaBuild_estudios.png" alt="Estudios" />
                </div>
                <div className={styles.dropdownText}>
            ...
            <Dropdown title={<span className={styles.dropdownTitleNormal}><b>Servicios</b></span>}>
              <div className={styles.dropdownNewContent}>
                <div className={styles.dropdownImage}>
                  <img src="/image/mediaBuild_servicios.png" alt="Servicios" />
                </div>
                <div className={styles.dropdownText}>
            ...
            <Dropdown title={<span className={styles.dropdownTitleNormal}><b>Alianzas Comerciales</b></span>}>
              <div className={styles.dropdownNewContent}>
                <div className={styles.dropdownImage}>
                  <img src="/image/mediaBuild_alianzasComerciales.png" alt="Alianzas Comerciales" />
                </div>
                <div className={styles.dropdownText}>
    ...
    // COMMENTED OUT - Fragment closing tag
    // </>
  );
}
...
function HechoConZeratypeContent() {
  return (
    <div className={styles.hechoConRoot}>
      <div className={`${styles.grid} ${styles.gridMobile}`}>
        {/* Descripción después del logo */}
        <div className={styles.col12}>
          <p className={styles.hechoConDescripcion}>
        ...
        <div className={`${styles.proyectosGrid} ${styles.col12}`}>
          ...
          <ProyectoCard
            ...
            header="/image/er_header.png"
            logoScale={1.75}
          />
          <ProyectoCard
            logo="/image/es_logo.svg"
            nombre="ES"
            seguidores={18750}
            likes={11200}
            comentarios={1580}
            descripcion="Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque euismod, nisi eu consectetur consectetur, nisl nisi consectetur nisi, euismod euismod nisi nisi euismod. Pellentesque euismod, nisi eu consectetur consectetur, nisl nisi consectetur nisi, euismod euismod nisi nisi euismod."
            instagram="@espacioseguro.ok"
            header="/image/es_header.png"
            logoScale={1.75}
          />
          <ProyectoCard
            logo="/image/pa_logo.svg"
            nombre="PA"
            seguidores={9450}
            likes={5320}
            comentarios={720}
            descripcion="Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque euismod, nisi eu consectetur consectetur, nisl nisi consectetur nisi, euismod euismod nisi nisi euismod. Pellentesque euismod, nisi eu consectetur consectetur, nisl nisi consectetur nisi, euismod euismod nisi nisi euismod."
            instagram="@paausactiva"
            header="/image/pa_header.png"
            logoScale={1.25}
          />
          <ProyectoCard
            logo="/image/prox_logo.svg"
        ...
        <div className={styles.col12}>
          <div className={styles.quieroSerParteWrapper}>
            <button className={styles.quieroSerParteButton}>
              Quiero ser parte
            </button>
          </div>
        </div>
...
function ProyectoCard({ logo, nombre, seguidores, likes, comentarios, descripcion, instagram, header, logoScale, disabled = false }) {
  ...
  return (
    <div className={`${styles.proyectoCard} ${disabled ? styles.proyectoCardDisabled : ''}`}>
      {/* 1. Logo del proyecto */}
      ...
      <div className={styles.proyectoStatsRow}>
        <div className={styles.proyectoIgButtonWrapper}>
          <a
            href={disabled ? undefined : `https://instagram.com/${instagram.replace('@', '')}`}
            target={disabled ? undefined : "_blank"}
            rel={disabled ? undefined : "noopener noreferrer"}
            className={`${styles.proyectoIgButton} ${disabled ? styles.proyectoIgButtonDisabled : ''}`}
      ...
      <div className={`${styles.grid} ${styles.gridMobile}`}>
        ...
        <div className={`${styles.contactoLeft} ${styles.col6} ${styles.mobileCol2}`}>
          ...
          <p className={styles.contactoDescripcion}>
            Lorem ipsum dolor sit amet consec tetur adipiscing elit aliquam
          </p>
          <div className={styles.contactoRedes}>
            <a href="https://instagram.com/zeratype_" target="_blank" rel="noopener noreferrer" className={styles.redSocial}>
            ...
            <a href="https://tiktok.com/@zeratype_" target="_blank" rel="noopener noreferrer" className={styles.redSocial}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className={`${styles.contactoRight} ${styles.col6} ${styles.mobileCol2}`}>
          <div className={styles.contactoMenu}>
            <div className={styles.menuColumna}>
              <a href="#hero" className={styles.menuLink}>Zeratype</a>
            ...
            <div className={styles.menuColumna}>
              ...
              <div className={styles.contactoItem}>
                ...
                <a href="mailto:contacto@zeratype.com">contacto@zeratype.com</a>
              </div>
              <div className={styles.contactoItem}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
